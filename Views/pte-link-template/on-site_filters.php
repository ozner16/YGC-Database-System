<?php
$region_select = "All Onsite links";
if (isset($_GET["selected_region"])) $region_select = $_GET["selected_region"];
?>
<section class="d-flex gap-2 py-3">

<form id="advance_filter" action="./on-site_index.php" method="GET" class="d-flex gap-2">
<select class="form-select w-max onchange-form-submit" name="selected_region" id="selected_region">
<option value="<?php $filter_region ?>">All Onsite links</option>
            <?php 
             $db->query("SELECT * from expo_details  where `is_online` = 'NO' group by expo_name");
              $db->execute();
