...
if (isset($_POST['edit-ps'])) {
  ...
  $db->query("SELECT * FROM event_category WHERE event_cat_name ='$status';");
  $db->execute();
  $db->closeStmt(); 
  if (sizeof($db->resultSet()) === 0){
  $db->query("UPDATE event_category SET `event_cat_name` ='$status' WHERE `id` = '$id';");
  $db->execute();
  $db->closeStmt();
...
<div class="modal fade modal-lg" id="editModal" tabindex="3" aria-labelledby="addModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <form action="./event-category-index.php" method="POST">
