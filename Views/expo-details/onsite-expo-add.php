...
$db = new Database();
  if (isset($_POST["add-on-site"])) {
    ...
    $db->query("SELECT MAX(slot_number) FROM expo_slots WHERE expo_id = '$expo_id';");
    $db->execute();
    $row_data = $db->fetch();
    $db->closeStmt();
    if (sizeof($db->resultSet()) > 0) {
        $check_max = $row_data["MAX(slot_number)"];

        $db->query("SELECT slot_count FROM expo_details WHERE id = '$expo_id';");
        $db->execute();
        $row_data = $db->fetch();
        $db->closeStmt();
        if (sizeof($db->resultSet()) > 0) {
            $check_maxcount = $row_data["slot_count"];

            if($check_max == $check_maxcount){
                $_SESSION["failed"] = "SLOT IS FULL.";
            }
            elseif($check_max != $check_maxcount){
                //check if slot number starts with 1 or 0
                $db->query("SELECT slot_number FROM expo_slots WHERE slot_number = '1' and expo_id = '$expo_id';");
                $db->execute();
                $row_data = $db->fetch();
                $check_user = $row_data["slot_number"];
                $db->closeStmt();
                
                if($check_user == 1){
                     //selects largest number then increment
                    $db->query("SELECT MAX(slot_number) FROM expo_slots JOIN expo_details on expo_details.id = expo_slots.expo_id  WHERE expo_id='$expo_id';");
                    $db->execute();
                    $row_data = $db->fetch();
                    $db->closeStmt();
                    $check_count = $row_data["MAX(slot_number)"];
                    $new_count = $check_count + 1;

                    $db->query("INSERT INTO requirement_for_expo_slot ( is_MOA, is_certificate, is_id, is_added_in_gc, suppliers_briefing, floor_layout, magazine, teaser_video, exhibitor_poster, purchasing_form, SOA, acknowledgement_receipt, exdeal, ingres, engres )
                    ...
                    $db->query("INSERT INTO expo_slots ( expo_id, business_id, package_id, requirement_id, slot_status, slot_name, slot_number ) VALUES ( '$expo_id', '$business_id', '$package', (SELECT MAX(id) FROM requirement_for_expo_slot), '$status', '$slot_name',
                    '$new_count');");
                    $db->execute();
                    $db->closeStmt();
                    ...
                    $_SESSION["failed"] = "Data adding failed. 4";
                }
            } else {
                $_SESSION["failed"] = "Data adding failed. 3";
            }
        } else {
            $_SESSION["failed"] = "Data adding failed. 2";
        }
    } else {
        $_SESSION["failed"] = "Data adding failed. 1";
    }
...
//             $_SESSION["success-on-site"] = "Data has been added successfully.";
//             echo '<script> window.location.href="./onsite-expo-index.php" </script>';
//           } else {
...
<div class="modal fade modal-lg" id="addModal" tabindex="3" aria-labelledby="addModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form action="./onsite-expo-index.php" method="POST" enctype="multipart/form-data">
